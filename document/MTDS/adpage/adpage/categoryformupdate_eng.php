...
if ($w == "")
{
    ...
    $sql = " insert g5_write_product_eng_category
                set ca_id   = '$ca_id',
                    ca_name = '$ca_name',
                    $sql_common ";
    sql_query($sql);
...
{
    $sql = " update g5_write_product_eng_category
                set ca_name = '$ca_name',
                    $sql_common
              where ca_id = '$ca_id' ";
    sql_query($sql);
...
else if ($w == "d")
{
    ...
    $sql = " select COUNT(*) as cnt from g5_write_product_eng_category
              where SUBSTRING(ca_id,1,$len) = '$ca_id'
                and ca_id <> '$ca_id' ";
    $row = sql_fetch($sql);
    if ($row['cnt'] > 0)
    ...
    // 분류 삭제
    $sql = " delete from g5_write_product_eng_category where ca_id = '$ca_id' ";
    sql_query($sql);
}

if ($w == "" || $w == "u")
{
    goto_url("./categoryform_eng.php?w=u&amp;ca_id=$ca_id&amp;$qstr");
} else {
    goto_url("./p_cate_eng.php?$qstr");
}
?>
