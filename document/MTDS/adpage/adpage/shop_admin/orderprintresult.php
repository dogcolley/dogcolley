...
if ($csv == 'xls')
{
    ...
    $sql = " SELECT a.od_id, od_b_zip1, od_b_zip2, od_b_addr1, od_b_addr2, od_b_addr3, od_b_addr_jibeon, od_b_name, od_b_tel, od_b_hp, b.it_name, ct_qty, b.it_id, a.od_id, od_memo, od_invoice, b.ct_option, b.ct_send_cost, b.it_sc_type, a.od_name, a.od_tel, a.od_hp, a.od_delivery_company, a.od_receipt_price, a.od_time, a.od_settle_case, a.od_receipt_time, a.od_status, a.od_deposit_name 
               FROM {$g5['g5_shop_order_table']} a, {$g5['g5_shop_cart_table']} b
              where a.od_id = b.od_id ";
    if ($case == 1) // 출력기간
        $sql .= " and a.od_time between '$fr_date 00:00:00' and '$to_date 23:59:59' ";
	...
	$data = array('주문번호','주문일자','상품명','수량','수취인명','연락처','핸드폰','우편번호','주소','메세지','주문자명','주문자연락처','주문자핸드폰','택배사','송장번호','','입금자명','결제금액','결제구분','결제일','주문상태');
    ...
    for($i=1; $row=sql_fetch_array($result); $i++)
    {
		...
		if($row['it_name'] == $row['ct_option']){
			$od_it_name = $row['it_name'];
		}else{
			$od_it_name = $row['it_name']."(".$row[ct_option].")";
		}

        $worksheet->write($i, 0, ' '.$row['od_id']);
        $worksheet->write($i, 1, substr($row['od_time'],0,10));
        $worksheet->write($i, 2, $od_it_name);
        $worksheet->write($i, 3, $row['ct_qty']);
        ...
        $worksheet->write($i, 9, $row['od_memo']);
        $worksheet->write($i, 10, $row['od_name']);
        $worksheet->write($i, 11, ' '.$row['od_tel']);
		$worksheet->write($i, 12, ' '.$row['od_hp']);
		$worksheet->write($i, 13, $row['od_delivery_company']);
		$worksheet->write($i, 14, $row['od_invoice']);
		$worksheet->write($i, 15, "");
		$worksheet->write($i, 16, $row['od_deposit_name']);
		$worksheet->write($i, 17, $row['od_receipt_price']);
		$worksheet->write($i, 18, $row['od_settle_case']);
		$worksheet->write($i, 19, substr($row['od_receipt_time'],0,10));
		$worksheet->write($i, 20, $row['od_status']);
    }
