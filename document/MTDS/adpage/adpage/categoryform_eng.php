...
$sql_common = " from g5_write_product_eng_category ";
if ($is_admin != 'super')
...
if ($w == "")
{
    ...
    $sql = " select MAX(SUBSTRING(ca_id,$len2,2)) as max_subid from g5_write_product_eng_category
    ...
    if ($ca_id) // 2단계이상 분류
    {
        $sql = " select * from g5_write_product_eng_category where ca_id = '$ca_id' ";
        $ca = sql_fetch($sql);
...
// 쿠폰 적용 불가 설정 필드 추가
if(!sql_query(" select ca_nocoupon from g5_write_product_eng_category limit 1 ", false)) {
    sql_query(" ALTER TABLE `g5_write_product_eng_category`
                    ADD `ca_nocoupon` tinyint(4) NOT NULL DEFAULT '0' AFTER `ca_adult_use` ", true);
}

// 스킨 디렉토리 필드 추가
if(!sql_query(" select ca_skin_dir from g5_write_product_eng_category limit 1 ", false)) {
    sql_query(" ALTER TABLE `g5_write_product_eng_category`
...
// 분류 출력순서 필드 추가
if(!sql_query(" select ca_order from g5_write_product_eng_category limit 1 ", false)) {
    sql_query(" ALTER TABLE `g5_write_product_eng_category`
                    ADD `ca_order` int(11) NOT NULL DEFAULT '0' AFTER `ca_name` ", true);
    sql_query(" ALTER TABLE `g5_write_product_eng_category` ADD INDEX(`ca_order`) ", true);
}

// 모바일 상품 출력줄수 필드 추가
if(!sql_query(" select ca_mobile_list_row from g5_write_product_eng_category limit 1 ", false)) {
    sql_query(" ALTER TABLE `g5_write_product_eng_category`
...
<form name="fcategoryform" action="./categoryformupdate_eng.php" onsubmit="return fcategoryformcheck(this);" method="post" enctype="multipart/form-data">
...
<section id="anc_scatefrm_basic">
    ...
    <div class="tbl_frm01 tbl_wrap">
		...
		<tr>
            <th scope="row">내용</th>
            <td>
                <?php echo editor_html('ca_1', get_text($ca['ca_1'], 0)); ?>
            </td>
        </tr>
		<? } ?>
        </tbody>
