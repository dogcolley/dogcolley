...
if ($_POST['date_area'] == "wr_datetime") {
    $sql = "SELECT * FROM {$g5['meta_table']} WHERE mta_db_table = '".$board['bo_1']."/reserved' AND DATE(mta_reg_dt) BETWEEN '{$_POST['startdt']}' AND '{$_POST['enddt']}' order by mta_key desc";
}else{
    $sql = "SELECT * FROM {$g5['meta_table']} WHERE mta_db_table = '".$board['bo_1']."/reserved' AND DATE(mta_key) BETWEEN '{$_POST['startdt']}' AND '{$_POST['enddt']}' order by mta_key desc";
}
