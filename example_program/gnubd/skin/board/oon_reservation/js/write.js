...
$('.multi-reserve-tab').click(function(event) {
    ...
    $('.multi-reserve-tab').removeClass('J_add_on');
    $('.check-submit2').find('span').text('객실을 선택하여 주세요.');
    $(this).find('.check-submit2 span').text('객실이 선택되었습니다.');
    $(this).addClass('J_add_on');
    // 다른거 클릭했을 시 슬라이드다운
    ...
    parent_wrap.find('.check-submit').hide();
    parent_wrap.find('.multi-reserve-tab').removeClass('submited-this-room');
...
$('.submit-btn-reserve').click(function(event) {
    if(typeof parent_wrap == 'string'){
        alert('객실을 선택해 주세요.');
        return false;
    }
    if( parent_wrap.data('wrid') !== $(this).parents('.multi-reserve-wrap').data('wrid')){
        alert('선택한 객실의 완료버튼을 눌러주세요!');
        return false;
    }
    ...
    totalPriceArray[parent_wrap.index()].checking = true;
    ...
    parent_wrap.find('.wr_4').datepicker("destroy"); 
    ...
    if ( end_date == '' ) {
        ...
        $.ajax( {
                ...
                success: function( data, textStatus ) {
                    ...
                    totalPriceArray[parent_wrap.index()].checking = false;
