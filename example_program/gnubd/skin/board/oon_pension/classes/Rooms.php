...
class Rooms extends Seasons
{
	...
	function __construct($bo_table, $wr_id = 0)
	{
		...
		$fetch = sql_fetch("SELECT * FROM {$this->g5['board_table']} WHERE bo_table = '$bo_table' AND bo_skin = 'theme/oon_pension'");

		if ($bo_table == $fetch['bo_table']) {
			$this->roomTableName = $bo_table;
			$this->board         = $board;
		}else{
            $newBoard = sql_fetch("SELECT * FROM {$this->g5['board_table']} WHERE bo_table = '{$bo_table}'");
		    $fetch = sql_fetch("SELECT * FROM {$this->g5['board_table']} WHERE bo_table = '".$newBoard['bo_1']."' AND bo_skin = 'theme/oon_pension'");
	...
	function searchRooms($startDate, $endDate, $type = "", $listIdArray = "", $selectedCountArray = "")
	{
		...
		while ($start <= $end){        
			...
			$sql = "SELECT * FROM {$this->g5['meta_table']} WHERE mta_db_table = '".$board['bo_1']."/reserved' AND mta_key='{$value}'";
