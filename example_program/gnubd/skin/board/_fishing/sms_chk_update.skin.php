...
if ($str_now <= $str_target) { // 지난 날짜는 문자 발송 건너뜀
    if ($config_set['pn_pg_sms_use'] == 1 && $uplus_sms['cf_nct_sms_use'] == 'uplus') {
		...
		for ($i = 0; $i < $chk_count; $i++) {
			$sql = " select wr_id, mb_id, ca_name, wr_name, wr_2, wr_3, wr_4, wr_5, wr_6, wr_7  from $write_table where wr_id = '" . $tmp_array[$i] . "' order by wr_id ";
            $result = @sql_fetch($sql);

            //$message_con = "";
			$sms_message = "";

			$use_hp_mb = $result["wr_4"]; // 사용자 전화번호
			...
			if ($use_smsid && $use_smskey && $remindCount > 0) {
				...
				if ($sms_type == "예약대기") {
					...
					if ($config_set["pn_sms_reserv_ready_adm"] == 1) {
						$sms_message = str_replace("{name}", $result["wr_name"], $config_set['pn_sms_reserv_ready_adm_info']);
						$sms_message = str_replace("{type}", $result["wr_3"], $sms_message);
						$sms_message = str_replace("{date}", $result["wr_2"], $sms_message);
						$sms_message = str_replace("{price}", number_format($result["wr_7"]) . "원", $sms_message);
						$sms_message = str_replace("{hp}", $result["wr_4"], $sms_message);
					}
				}
				if ($sms_type == "예약완료") {

					if ($config_set["pn_sms_reserv_comp_adm"] == 1) {
						$sms_message = str_replace("{name}", $result["wr_name"], $config_set['pn_sms_reserv_comp_adm_info']);
						$sms_message = str_replace("{type}", $result["wr_3"], $sms_message);
						$sms_message = str_replace("{date}", $result["wr_2"], $sms_message);
						$sms_message = str_replace("{price}", number_format($result["wr_7"]) . "원", $sms_message);
						$sms_message = str_replace("{hp}", $result["wr_4"], $sms_message);
				...
				if ($sms_type == "취소요청") {
					...
					if ($config_set["pn_sms_reserv_canc_adm"] == 1) {
						$sms_message = str_replace("{name}", $result["wr_name"], $config_set['pn_sms_reserv_canc_adm_info']);
						$sms_message = str_replace("{type}", $result["wr_3"], $sms_message);
						$sms_message = str_replace("{date}", $result["wr_2"], $sms_message);
						$sms_message = str_replace("{price}", number_format($result["wr_7"]) . "원", $sms_message);
						$sms_message = str_replace("{hp}", $result["wr_4"], $sms_message);
				...
				if ($sms_type == "취소완료") {
					...
					if ($config_set["pn_sms_reserv_canc_cplt_adm"] == 1) {
						$sms_message = str_replace("{name}", $result["wr_name"], $config_set['pn_sms_reserv_canc_cplt_adm_info']);
						$sms_message = str_replace("{type}", $result["wr_3"], $sms_message);
						$sms_message = str_replace("{date}", $result["wr_2"], $sms_message);
						$sms_message = str_replace("{price}", number_format($result["wr_7"]) . "원", $sms_message);
						$sms_message = str_replace("{hp}", $result["wr_4"], $sms_message);
				...
				if ($sms_message) {
					if (mb_strlen($sms_message) >= 90)
						$info = @parse_url("http://sms.nccj.kr/api/lms");
					else
			...
			if ($use_smsid && $use_smskey && $remindCount > 0) {
				...
				if ($sms_type == "예약대기") {
					...
					if ($config_set["pn_sms_reserv_ready_user"] == 1) {
						$sms_message = str_replace("{name}", $result["wr_name"], $config_set['pn_sms_reserv_ready_user_info']);
						$sms_message = str_replace("{type}", $result["wr_3"], $sms_message);
						$sms_message = str_replace("{date}", $result["wr_2"], $sms_message);
						$sms_message = str_replace("{price}", number_format($result["wr_7"]) . "원", $sms_message);
						$sms_message = str_replace("{hp}", $result["wr_4"], $sms_message);
				...
				if ($sms_type == "예약완료") {
					...
					if ($config_set["pn_sms_reserv_comp_user"] == 1) {
						$sms_message = str_replace("{name}", $result["wr_name"], $config_set['pn_sms_reserv_comp_user_info']);
						$sms_message = str_replace("{type}", $result["wr_3"], $sms_message);
						$sms_message = str_replace("{date}", $result["wr_2"], $sms_message);
						$sms_message = str_replace("{price}", number_format($result["wr_7"]) . "원", $sms_message);
						$sms_message = str_replace("{hp}", $result["wr_4"], $sms_message);
				...
				if ($sms_type == "취소요청") {
					...
					if ($config_set["pn_sms_reserv_canc_user"] == 1) {
						$sms_message = str_replace("{name}", $result["wr_name"], $config_set['pn_sms_reserv_canc_user_info']);
						$sms_message = str_replace("{type}", $result["wr_3"], $sms_message);
						$sms_message = str_replace("{date}", $result["wr_2"], $sms_message);
						$sms_message = str_replace("{price}", number_format($result["wr_7"]) . "원", $sms_message);
						$sms_message = str_replace("{hp}", $result["wr_4"], $sms_message);
				...
				if ($sms_type == "취소완료") {
					...
					if ($config_set["pn_sms_reserv_canc_cplt_user"] == 1) {
						$sms_message = str_replace("{name}", $result["wr_name"], $config_set['pn_sms_reserv_canc_cplt_user_info']);
						$sms_message = str_replace("{type}", $result["wr_3"], $sms_message);
						$sms_message = str_replace("{date}", $result["wr_2"], $sms_message);
						$sms_message = str_replace("{price}", number_format($result["wr_7"]) . "원", $sms_message);
						$sms_message = str_replace("{hp}", $result["wr_4"], $sms_message);
