...
// 지나간날짜와 신청날짜 비교
if($str_now > $str_target && $member[mb_level] < 8) {
...
unset($arr_db);
$arr_db = array();
$sql = "select wr_id, wr_subject, ca_name, wr_name, wr_1, wr_2, wr_3, wr_4, wr_5, wr_6, wr_7, wr_8, wr_9, wr_10 from {$write_table} where wr_2 = '{$wr_2}' and wr_3 = '{$wr_3}' and (ca_name != '공지' and ca_name not like '%취소%') order by wr_3 asc";
$res = sql_query($sql);
...
// 독배 예약이 1개라도 있다면 예약 금지 (관리자만)
if($dokbae && $member['mb_level'] < 8) {
	alert('독배예약으로 이미 마감되었습니다.\n다시 확인해주시기 바랍니다.', G5_HTTP_BBS_URL.'/board.php?bo_table='.$bo_table.'&amp;sch_year='.$dArr[0].'&amp;sch_month='.$dArr[1].'#'.$wr_2);
	exit;
}
...
for($i=0; $i<count($bArr)-1; $i++) {
	...
	for ($z=0; $z < $cnt_db; $z++) {
		if($sArr[1] == $arr_db[$z]['wr_3']){
			$all_cnt = $sArr[2];
		}
		if($arr_db[$z]['wr_6'] && $arr_db[$z]['wr_3'] == $sArr[1]){
...
if($all_cnt) {
	$all_cnt = $all_cnt; // 정원
	$left_cnt = $all_cnt - $p_cnt; // 잔여
	$total_cnt = $p_cnt + $wr_6; 
}else{
	$all_cnt = $_POST[total_per];
	$left_cnt = $all_cnt - $p_cnt;
	$total_cnt = $p_cnt + $wr_6;
}


if($w == "u") {
	$uRow = sql_fetch("select wr_id, wr_subject, ca_name, wr_name, wr_1, wr_2, wr_3, wr_4, wr_5, wr_6, wr_7 from {$write_table} where wr_id = '{$wr_id}'");
	
	$now_cnt = $wr_6 - $uRow[wr_6];
	
	// 동시에 예약 할 경우에 잔여 인원수 체크
	if($left_cnt < $now_cnt) 
	{
		alert('이미 마감되었거나 잔여 인원을 초과하였습니다.\n다시 확인해주시기 바랍니다.', G5_HTTP_BBS_URL.'/board.php?bo_table='.$bo_table.'&amp;sch_year='.$dArr[0].'&amp;sch_month='.$dArr[1].'#'.$wr_2);
		exit;
	}
