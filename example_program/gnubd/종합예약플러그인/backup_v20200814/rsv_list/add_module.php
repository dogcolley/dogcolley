...
//컬럼추가
$check_cul = sql_query("SHOW COLUMNS FROM `${write_table}` LIKE 'wr_12'");
if($check_cul -> num_rows == 0) {
	$sql =" ALTER TABLE `{$write_table}`
	ADD `wr_12` DATETIME NOT NULL AFTER `wr_11`";
	sql_query($sql);
}

//컬럼추가
$check_cul = sql_query("SHOW COLUMNS FROM `${write_table}` LIKE 'wr_13'");
if($check_cul -> num_rows == 0) {
	$sql =" ALTER TABLE `{$write_table}`
	ADD `wr_13` DATETIME NOT NULL AFTER `wr_12`";
	sql_query($sql);
}

//컬럼추가
$check_cul = sql_query("SHOW COLUMNS FROM `${write_table}` LIKE 'wr_14'");
if($check_cul -> num_rows == 0) {
	$sql =" ALTER TABLE `{$write_table}`
	ADD `wr_14` DATETIME NOT NULL AFTER `wr_13`";
	sql_query($sql);
}
...
function hd_check2($ch_arr,$check_data,$set_day,$rsv_arr){
	...
	if(ch_limo($set_day) == 'false'){
		$data['ch_status'] = '예약불가';
		$str_ch = false;
	}else {
		if(count($ch_arr) < 1 && $str_now >= $str_today){
			$data['ch_status'] = '예약가능';
		}else{
			for($i=0;$i < count($ch_arr); $i++){
				...
				if(($str_now >= $str_min && $str_max >= $str_now)) {
					...
					if($ch_arr[$i]['wr_2'] == '' || $ch_arr[$i]['wr_2'] == $check_data['wr_id']){
						$data['ch_status'] = '예약불가';
						if($ch_arr[$i]['wr_2'] == '')
							$data['wr_subject'] = $ch_arr[$i]['ca_name'];
						else
							$data['wr_subject'] = $data['wr_subject'].' '.$ch_arr[$i]['ca_name'];
						$data['wr_ca'] = $ch_arr[$i]['ca_name'];
						$str_ch = false;
						$cnt = 0;
						continue;
					}
				}else if($str_ch && $data['ch_status'] !=='예약불가'){
					$data['ch_status'] = '예약가능';
					$str_ch = false;
					continue;
				}
...
function ch_member($wr_id){
	...
	$sql = "SELECT wr_name,wr_id,wr_1,wr_6,wr_4,wr_13 FROM ".$write_table." where wr_1 like '%".$wr_id."%' AND wr_2 = '".$set_day."' AND wr_4 != '예약취소' ";
	$qry = sql_query($sql);
...
function ch_limo($set_day = null){
	...
	*/
	...
	if($board['bo_2'] == "1"){ //그달 매칭
		...
		if($set_year == $match_year){
            if($match_month == $set_month)$state = 'true';
            else if($match_month+1 == $set_month || ($match_month == 12 && $set_month == 1)){
                $state = 'true';
...
function get_name($set_nick){
	$data = sql_fetch('SELECT mb_name FROM g5_member WHERE mb_nick = "'.$set_nick.'"');
	return $data['mb_name'];
}
