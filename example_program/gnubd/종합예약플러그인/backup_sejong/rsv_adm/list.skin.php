...
<div id="bo_list<?php if ($is_admin) echo "_admin"; ?>">
	...
	<div class="U_wrap01">
		...
		<form action="<?=$url?>" id="ch_lbo" name="ch_lbo" method="post">
			...
			<div class="TM_ds_inbl">
				...
				<input readonly type="text" id="lbo_table" name="lbo_table" value="<?=$board['bo_1']?>" class="required frm_input"/>
	...
	<p style="background:#f9f9f9;margin-bottom:40px;padding:20px 30px;line-height:2">
		<strong style="display:block;margin-bottom:20px;color:#000">주의하실사항</strong>
		1. 휴일처리는 <span style="color:red">'휴일관리'</span>, 예약처리는 <span style="color:red">'예약페이지'</span>, 예약등록은 <span style="color:red">'예약관리'</span> 에서 진행해 주시면 됩니다.
		<br/>
		2. 상시 노출되는 예약을 가리거나 휴일 처리하고 싶으면 반드시 <span style="color:red">휴일설정 게시판에서 휴일을</span> 설정해주세요.
		<br/>
		3. 관리자는 날짜에 구애 받지 않고 예약 할 수 있습니다. 미리 예약을 해두고 싶으면 예약페이지에서 예약을 해주세요. 휴일로 해당 예약을 처리할경우 예약정보는 입력 할 수 없습니다.
		<br/>
		4. 예약시 인원을 원하는 인원만큼 꼭 입력해주세요. 입력하지 않을경우 해당 예약을 무수히 많은 사람이 예약을 할 수 있습니다. 
	</p>
    ...
    <div class="tbl_head01 tbl_wrap">
        ...
        <tr>
            ...
            <th scope="col" style="width:530px;">예약시간</th>
			<th>일정기간</th>
			<th>진행시간</th>
			<th>인원제한</th>
        </tr>
		...
		$wr_12  = array_column($list, 'wr_12');

		array_multisort($wr_12,SORT_ASC,$list);
